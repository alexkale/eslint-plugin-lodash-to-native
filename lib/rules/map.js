  ...
  create: (context) => {
    ...
    */
    const isWrappedInConditional = (node) => {
      ...
      && node.parent.test.arguments[0].name === node.arguments[0].name) return true;
      return false;
    };
    ...
    const getReplacementText = (callee, iife = false) => {
      ...
      const ifCondition = `Array.isArray(${iife ? 'val' : collectionSrc})`;
      const lodashCall = `_.map(${iife ? 'val' : collectionSrc}, ${cbSrc})`;
      const nativeCall = `${iife ? 'val' : collectionSrc}.map(${cbSrc})`;
    ...
    return {
      ...
      "CallExpression[callee.type='MemberExpression'][callee.object.name='_'][callee.property.name='map'][arguments]": function (node) {
        ...
        if (isIdentifier(collection)) {
        ...
        } else if (isArrayExpression(collection)) {
          context.report({
            node,
            message: 'Use native Array.map implementation instead of lodash version for Array objects',
            fix(fixer) {
          ...
          context.report({
            node,
            message: 'Use native Array.map implementation instead of lodash version (with isArray check and caching)',
            fix(fixer) {
